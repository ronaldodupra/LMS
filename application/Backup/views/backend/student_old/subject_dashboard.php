        ...
        <div class="row">
          <main class="col col-xl-6 order-xl-2 col-lg-12 order-lg-1 col-md-12 col-sm-12 col-12">
            <div id="newsfeed-items-grid">     
              <?php 
                $db = $this->db->query("SELECT homework_id, wall_type,publish_date FROM homework WHERE class_id = $ex[0] AND subject_id = $ex[2] AND section_id = $ex[1] UNION SELECT document_id,wall_type,publish_date FROM document WHERE class_id = $ex[0] AND subject_id = $ex[2] AND section_id = $ex[1] UNION 
                  SELECT online_exam_id,wall_type,publish_date FROM online_exam WHERE class_id = $ex[0] AND subject_id = $ex[2] AND section_id = $ex[1] UNION SELECT post_id,wall_type,publish_date FROM forum 
                  WHERE class_id = $ex[0] AND subject_id = $ex[2] AND section_id = $ex[1] ORDER BY publish_date DESC LIMIT 10");
                if($db->num_rows() > 0):
                ...
                <div class="sidebar__inner">
                    ...
                    <div class="ui-block">
                      <div class="ui-block-title">
                        <h6 class="title">Actividad del Curso</h6>                      
                      </div>
                       <?php 
...
$(document).ready(function(){
  ...
  $(document).on('click', '.btn_archive', function(){
    var id = $(this).attr('id');
      $.ajax({
        url:"<?php //echo base_url();?>admin/insert_archive",
        method:"POST",
        data:{id:id},
