                          ...
                          <div class="col-sm-12">
                            ...
                            <div class="row">
                              ...
                              <div class="col col-lg-4 col-md-4 col-sm-12 col-12">
                                <button type="button" class="btn btn-success" onclick="add_student('<?php echo $department_id;?>','<?php echo $class_id; ?>','<?php echo $section_id; ?>','<?php echo $subject_id; ?>')"><span class="fa fa-plus fa-lg"></span> Add
                                </button>
                                <button type="button" class="btn btn-info" onclick="stud_exld_list();"><span class="fa fa-list fa-lg"></span> Excluded List
                                </button>
                            ...
                            <div class="row">
                              <?php
                                ...
                                else
                                {
                                  ...
                                  $students = $this->db->query("SELECT * FROM student_list_elem_hs WHERE subject_id = '$subject_id' AND status = '1' AND year = '$running_year'");
                              ...
                              <div class="col-xl-4 col-md-6" id="results">
                                <div class="card-box widget-user ui-block list">
                                  <?php
                                    ...
                                    $chk_stud_added = $this->db->query("SELECT student_id FROM tbl_students_subject WHERE student_id = '$stud_id' AND subject_id = '$subject_id' AND year = '$running_year'")->num_rows();
                                  ?>
...
<div class="modal fade" id="bulkstudents" tabindex="-1" role="dialog" aria-labelledby="bulkstudents" aria-hidden="true">
  <div class="modal-dialog window-popup fav-page-popup" role="document" style="width: 60%;">
      ...
      <form enctype="multipart/form-data" id="student_exclude" onsubmit="event.preventDefault();">
        ...
        <div class="modal-body">
            ...
            <div class="col col-lg-12 col-md-12 col-sm-12 col-12">
              <div class="form-group">
                <div class="form-buttons-w text-right">
                  <center><button class="btn btn-rounded btn-success btn-lg full-width" id="btn_add_stud" onclick="add_subj_student('<?php echo $param_data; ?>');" type="submit" disabled=""><?php echo get_phrase('add_student');?></button></center>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="row" id="con_list">
            ...
            <div class="col col-lg-12 col-md-12 col-sm-12 col-12">
              <div class="form-group" id="student">
                <div class="col-md-12" style="height: 350px; overflow: auto;" id="tbl_member_list"></div> 
              </div>
            </div>
            <div class="col col-lg-12 col-md-12 col-sm-12 col-12">
              <div class="form-group">
...
<div class="modal fade" id="stud_excluded_list" tabindex="-1" role="dialog" aria-labelledby="bulkstudents" aria-hidden="true">
  <div class="modal-dialog window-popup fav-page-popup" role="document" style="width: 40%;">
    <div class="modal-content">
      <a href="#" class="close icon-close" data-dismiss="modal" aria-label="Close"></a>
        <div class="modal-header">
          <h6 class="title"><?php echo get_phrase('excluded_student_lists');?></h6>
        </div>
        <div class="modal-body">
          <div class="table-responsive" style="overflow: auto; height: 550px; ">
            <table class="table table-lightborder table-striped">
              ...
              <tbody>
                <?php
                  ...
                  $list = $this->db->query("SELECT t1.`Id`, t1.`student_id`, CONCAT(t2.`last_name`,', ',t2.`first_name`) AS fullname, t3.`name` AS grade , t4.`name` as section FROM tbl_stud_subject_exclusion t1 LEFT JOIN student t2 ON t1.`student_id` = t2.`student_id`LEFT JOIN class t3 ON t1.`class_id` = t3.`class_id` LEFT JOIN section t4 ON t1.`section_id` = t4.`section_id` WHERE t1.`section_id` = '$section_id' AND t1.`subject_id` = '$subject_id' ORDER BY t2.`last_name` ASC")->result_array();

                  foreach ($list as $row_list):

                ?>
                <tr>
                  <td><?php echo $row_list['fullname']; ?></td>
                  <td><?php echo $row_list['grade']; ?></td>
                  <td><?php echo $row_list['section']; ?></td>
                  <td>
                    <a href="javascript:void(0);" onclick="include_stud('<?php echo $row_list['Id'];?>','<?php echo $param_data;?>')" class="btn btn-primary btn-sm btn-rounded"><?php echo get_phrase('include');?></a>
                  </td>
                </tr>
                <?php endforeach; ?>
              </tbody>
      ...
      function(isConfirm){
        ...
        {  
          ...
          else
          {
            ...
            $.ajax({
              url:'<?php echo base_url();?>admin/multiple_exclude_student/',
              method:'POST',
              ...
              success:function(data)
              {
                ...
                if(data == 404){
                 ...
                 swal("LMS", "Error on rejecting data", "info");
                  ...
                  window.location.href = '<?php echo base_url();?>admin/students_enrolled/' + param;
...
<script type="text/javascript">
  ...
  function add_student(dept_id,class_id,section_id,subject_id)
  {
    $("#con_list").hide('fast');
    if (dept_id == 4) {
      load_student_info(dept_id,class_id,section_id,subject_id);
    }
    
    $('#bulkstudents').modal('show');
  }

  function stud_exld_list()
  {
    $('#stud_excluded_list').modal('show');
  }
  ...
  { 
    ...
    $("#con_list").show('fast');
    load_student_info(dept_id,class_id,section_id,subj_id)
  }

  function load_student_info(dept_id,cl_id,sec_id,subj_id)
  {   

    $.ajax({
      url: '<?php echo base_url();?>admin/load_student_list/' + cl_id + '/' + sec_id + '/' + subj_id + '/' + dept_id,
      success: function(response)
      {
        jQuery('#tbl_member_list').html(response);
      }
    });
    return false;
  ...
  function add_subj_student($param_data) {
    ...
    swal({
      title: "Are you sure?",
      text: "You want to add selected students to your subject?",
      type: "warning",
      showCancelButton: true,
      confirmButtonColor: "#34464a",
      confirmButtonText: "Yes",
    ...
    function(isConfirm){
      ...
      {  
        ...
        $.ajax({
          url:'<?php echo base_url();?>admin/students_enrolled/Add/' + $param_data,
          method:'POST',
          data:{id:id},
          cache:false,
          success:function(data)
  ...
  function include_stud($id, $param) {

    swal({
      title: "Are you sure?",
      text: "You want to include this student to your list again?",
      type: "warning",
      showCancelButton: true,
      confirmButtonColor: "#34464a",
      confirmButtonText: "Yes",
    ...
    function(isConfirm){
      ...
      {  
        ...
        $.ajax({
          url:'<?php echo base_url();?>admin/include_student/',
          method:'POST',
          data:{Id:Id},
          cache:false, 
          success:function(data)
          {
            ...
            if(data == 1){
              ...
              swal("LMS", "Selected student successfully included.", "success");
              window.location.href = '<?php echo base_url();?>admin/students_enrolled/' + $param;
  
            }else{
  
              swal("LMS", "Error on including student", "info");
