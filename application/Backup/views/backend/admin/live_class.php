                                 ...
                                 <div class="table-responsive" style="margin-top: -2%;">
                                    <table class="table table-padded">
                                      ...
                                      <?php
                                        ...
                                        $live_class_info = $this->db->query("SELECT t2.`name` AS semester_name, t1.*, t3.`zoom_id`, t3.`zoom_password`, type FROM tbl_live_class t1 LEFT JOIN exam t2 ON t1.`semester_id` = t2.`exam_id` LEFT JOIN teacher t3 ON t1.`teacher_id` = t3.`teacher_id` WHERE subject_id = '$subject_id' AND class_id = '$class_id' AND section_id = $ex[1] ORDER BY live_id DESC");
                                        
                                        if ($live_class_info->num_rows() > 0):
                                        
                                          foreach($live_class_info->result_array() as $rows):
                                        ...
                                        <tr>
                                          ...
                                          <td class="text-center bolder">
                                             ...
                                             <?php if ($rows['host_id'] == 2): ?>
                                                ...
                                                <a title="Join Live Classroom" href="<?php echo base_url();?>jitsi_meet/client.php?data=<?php echo base64_encode($room_name.'-'.$sname);?>" target="_blank" class="btn btn-info btn-sm laptop_desktop"> <i class="picons-thin-icon-thin-0324_computer_screen"></i> <?php echo get_phrase('Guest');?></a>

                                                <a title="Join Live Classroom" href="https://meet.jit.si/<?php echo $room_name?>" target="_blank" class="btn btn-info btn-sm mobile"> <i class="picons-thin-icon-thin-0191_window_application_cursor"></i> <?php echo get_phrase('Guest');?></a>
                                             <?php else:

                                                $zoom_link=$this->db->get_where('teacher', array('teacher_id' => $teacher_id))->row()->zoom_link; 
                                             ?>

                                               <a title="Join Live Classroom" href="<?php echo $zoom_link ?>" target="_blank" class="btn btn-info btn-sm"> <i class="picons-thin-icon-thin-0191_window_application_cursor"></i><?php echo get_phrase('Guest');?></a>
                                                 
                                            <?php endif ?>

                                              <a href="javascript:void(0)" onclick="showAjaxModal('<?php echo base_url();?>modal/popup/update_live_class/<?php echo $row['live_id']?>/<?php echo $data;?>')" class="btn btn-success btn-sm"><i class="picons-thin-icon-thin-0001_compose_write_pencil_new"></i> <?php echo get_phrase('edit');?></a><br>

                                              <a href="javascript:void(0)" onclick="delete_data2('<?php echo $row['live_id'] ?>','<?php echo $data ?>')" class="btn btn-danger btn-sm"><i class="picons-thin-icon-thin-0056_bin_trash_recycle_delete_garbage_empty"></i> <?php echo get_phrase('delete');?></a>
