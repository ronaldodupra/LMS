                     ...
                     <div class="element-wrapper">
                        <div class="element-box-tp">
                           <h5 class="element-header">
                              ...
                              <div style="margin-top:auto;float:right;" class="mr-5">
                                 <a href="#new_exam_modal" data-toggle='modal' class="text-white btn btn-control btn-grey-lighter btn-success">
                                   ...
                                   <tbody id="results">
                                      <?php
                                         ...
                                         $online_exams = $this->db->query("SELECT * from online_exam 
                                          where running_year = '$year' and subject_id = '$subject_id' and class_id = '$class_id' and section_id = $ex[1] and semester_id = '$semester_id' order by online_exam_id asc");
                                      ...
                                      <tr>
                                         ...
                                         <td><span><?php echo '<b>'.get_phrase('date').':</b> '.date('M d, Y', $row['exam_date']).'<br>'.'<b>'.get_phrase('hour').':</b> '.date('g:i A', strtotime($row['time_start'])).' - '.date('g:i A', strtotime($row['time_end']));?></span></td>
                                         <td class="bolder">

                                            <a href="<?php echo base_url();?>admin/examroom/<?php echo $row['online_exam_id'];?>" class="btn btn-success btn-sm"> <?php echo get_phrase('details');?></a>
                                            ...
                                            <a onclick="mark_expired_data('<?php echo $row['online_exam_id'];?>','<?php echo $data;?>')" href="#" class="btn btn-primary btn-sm"> <?php echo get_phrase('mark_as_expired');?></a><br>
                                            ...
                                            if ($row['is_view'] == 0){ ?>
                                              ...
                                              <a href="#" onclick="enable_result('<?php echo $row['online_exam_id'] ?>','<?php echo $data;?>');" class="btn btn-success  btn-sm"> <?php echo get_phrase('enable_result');?></a>

                                            <?php }else{ ?>

                                              <a href="#" onclick="disable_result('<?php echo $row['online_exam_id'] ?>','<?php echo $data;?>');" class="btn btn-danger btn-sm"> <?php echo get_phrase('disable_result');?></a>
                                            ...
                                            <a onclick="delete_data2('<?php echo $row['online_exam_id'];?>','<?php echo $data;?>')" class="btn btn-danger btn-sm" href="#"><?php echo get_phrase('delete');?></a>
  ...
  function publish_data(exam_id,data) {
     ...
     function(isConfirm){
       ...
       {        
         ...
         window.location.href = '<?php echo base_url();?>admin/manage_online_exam_status/' + exam_id + '/published/' + data +'/';
   ...
   function mark_expired_data(exam_id,data) {
     ...
     function(isConfirm){
       ...
       {        
         ...
         window.location.href = '<?php echo base_url();?>admin/manage_online_exam_status/' + exam_id + '/expired/' + data +'/';
     ...
     function(isConfirm){
       ...
       {        
        ...
        $.ajax({
            ...
            url:"<?php echo base_url();?>admin/check_exams/",
            type:'POST',
            ...
            success:function(result)
            {
              ...
              if(result == 0){
                ...
                swal('info','You cannot delete this record it has existing related information.','info');
                 ...
                 window.location.href = '<?php echo base_url();?>admin/manage_exams/delete/' + exam_id +'/'+ data;
