      ...
      <div class="rcard-wy" id="print_area">
        <div class="rcard-w">
          <div class="infos">
            ...
            <img alt="" src="<?php echo $this->crud_model->get_image_url('student', $ex[0]);?>">
            </div>
              <div class="company-name"><?php echo $this->crud_model->get_name('student' , $ex[0]);?></div>
              <div class="company-address">
                <?php echo get_phrase('roll');?>: <?php echo $this->db->get_where('enroll', array('student_id' => $ex[0]))->row()->roll;?><br/><?php echo $this->db->get_where('class', array('class_id' => $class_id))->row()->name;?><br/><?php echo $this->db->get_where('section' , array('class_id' => $class_id))->row()->name;?>
              </div>
            </div>
          </div>
              ...
              <table class="table">
                ...
                <tbody>
                 <?php 
                    ...
                    $section = $this->db->get_where('enroll', array('student_id' => $ex[0]))->row()->section_id;
                    $subjects = $this->db->get_where('subject' , array('class_id' => $class_id, 'section_id' => $section))->result_array();
                    foreach ($subjects as $row3):
                    $mark = $this->db->get_where('mark' , array('student_id' => $ex[0],'subject_id' => $row3['subject_id'], 'class_id' => $class_id, 'exam_id' => $ex[1], 
