                  ...
                  <select name="reciever" id="slct" required="">
                     ...
                     <optgroup label="<?php echo get_phrase('teachers');?>">
                        <?php
                           ...
                           $teachers = $this->db->query("SELECT t1.`last_name`,t4.* FROM student t1
                                                       RIGHT JOIN enroll t2 ON t1.`student_id` = t2.`student_id`
                                                       RIGHT JOIN class t3 ON t2.`class_id` = t3.`class_id`
                                                       RIGHT JOIN teacher t4 ON t3.`teacher_id` = t4.`teacher_id`
                                                       WHERE t1.`parent_id` = '$login_id' GROUP BY t4.`teacher_id` order by t4.last_name asc");
                     ...
                     <optgroup label="<?php echo get_phrase('students');?>">
                        <?php
                           $students = $this->db->query("SELECT * from student where parent_id = '$login_id' order by last_name asc");
