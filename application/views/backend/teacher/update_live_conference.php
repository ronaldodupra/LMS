...
<div class="modal-body">
  ...
  <div class="ui-block-content">
    <?php echo form_open(base_url() . 'teacher/live_conference/update/'.$live_id.'/'.$data.'/'.$form  , array('enctype' => 'multipart/form-data'));?>
    <div class="row">
      <?php echo "Type ".$query['type']; ?>
      ...
      <div class="col col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="form-group">
          ...
          <div class="select">
            ...
            <select name="type_id" required="" id="type_id" onload="select_type(this.value)" onchange="select_type(this.value)">
      ...
      <?php if($query['type'] == 'teacher'):?>
      <div class="col col-lg-6 col-md-6 col-sm-12 col-12" id="econ_cl">
        <div class="form-group">
          <label class="control-label"><?php echo get_phrase('select_class');?></label>
          <div class="select">
            <select name="class_id" id="cl_id" required="" onchange="subject_holder(this.value);">
              <option value=""><?php echo get_phrase('select');?></option>
              <?php 
                $classes = $this->db->query("SELECT * FROM class ORDER BY nivel_id ASC")->result_array();
                foreach($classes as $row):
              ?>
              <option value="<?php echo $row['class_id'];?>" <?php if($row['class_id'] == $query['class_id']) echo 'selected';?>><?php echo $row['name'];?></option>
              <?php endforeach;?>
            </select>
          </div>
        </div>
      ...
      <div class="col col-lg-6 col-md-6 col-sm-12 col-12" id="econ_sec">
        <div class="form-group">
          ...
          <div class="select">
            <select name="section_id" required="">
              <option value=""><?php echo get_phrase('select');?></option>
              <?php 
                $class_info = $this->db->get_where('section' , array('class_id' => $query['class_id']));
                if ($class_info->num_rows() > 0):
                  $sections = $class_info->result_array();
                  foreach ($sections as $rowd) {?>
                     <option value="<?php echo $rowd['section_id']; ?>" <?php if($query['section_id'] == $rowd['section_id']) echo 'selected';?>><?php echo $rowd['name']; ?></option>
                  <?php };?>
              <?php endif;?>
            </select>
          </div>
