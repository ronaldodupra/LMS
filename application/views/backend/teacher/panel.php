...
$class_id = $this->db->query("SELECT t1.* FROM section t1 where teacher_id = '$login_id'")->row()->class_id;
...
$query_student = $this->db->query("SELECT t1.*, t2.student_id FROM online_users t1
             ...
             WHERE t2.`class_id` = '$class_id' AND section_id = '$section_id' AND t1.`type` = 'student'
             GROUP BY t1.`gp`")->num_rows();

$user_counter =  $query_student;

$date_today = date('Y-m-d');

$date_today_live_class = date('m/d/Y');

$online_exam_today = $this->db->query("SELECT * FROM online_exam WHERE uploader_id = '$login_id' and examdate = '$date_today' order by time_start ASC");

$online_quiz_today = $this->db->query("SELECT * FROM tbl_online_quiz WHERE uploader_id = '$login_id' and quizdate = '$date_today' order by time_start ASC");

$activity_today = $this->db->query("SELECT * FROM homework WHERE uploader_id = '$login_id' and (date_end = '$date_today' or date_end = '$date_today_live_class')  ORDER BY time_end ASC");

$live_class_today = $this->db->query("SELECT * FROM tbl_live_class WHERE teacher_id = '$login_id' and (start_date = '$date_today_live_class' or start_date = '$date_today')  ORDER BY start_time ASC");
      ...
      <div class="row">
         ...
         <div class="ui-block paddingtel">
            <div class="widget w-create-fav-page">
               ...
               <div class="content ">
                  <h4 class="title text-center"> <i class="picons-thin-icon-thin-0729_student_degree_science_university_school_graduate text-white" style="font-size:25px;"></i>&nbsp; <?php echo get_phrase('welcome_teacher_dashboard');?></h4>
                  
                  <h6 class="text-center">
                     <a href="<?php echo base_url();?>teacher/grados/" class="btn btn-warning ">
                        <i class="picons-thin-icon-thin-0680_pencil_ruller_drawing"></i> <?php echo get_phrase('go_to_my_classes');?></a>
                  </h6>
         ...
         <!-- ONLINE EXAM TODAY -->
         <?php if($online_exam_today->num_rows() > 0){ ?>
            <div class="ui-block paddingtel" >
            <div class="pipeline white lined-success" >
               <div class="element-wrapper" >
                  ...
                  <div class="full-ch at-w" id="results">
                           ...
                           <div class="users-list-w">
                              ...
                              if($online_exam_today->num_rows() > 0){
                                 ...
                                 foreach($quizzes as $rowe): ?>
                                    ...
                                    $exam_id = $rowe['online_exam_id'];

                                    $failed = $this->db->query("SELECT * from online_exam_result where online_exam_id = '$exam_id' and result = 'fail'")->num_rows();

                                    $passed = $this->db->query("SELECT * from online_exam_result where online_exam_id = '$exam_id' and result = 'pass'")->num_rows();
                                    ...
                                    $total_examiner = $total_students - $failed - $passed;

                                    ?>
                                       
                                    <div class="user-name">
                                       <a href="<?php echo base_url().'teacher/exam_results/'.$rowe['online_exam_id']; ?>" target="_blank">
                                          <h6 class="user-title min"><?php echo $rowe['title'];?> <br>
                                             <span class="badge badge-success"><span class="fa fa-clock"></span> <?php echo date('g:i A', strtotime($rowe['start_date'])) . ' - ' . date('g:i A', strtotime($rowe['end_date'])); ?> </span>
                                          ...
                                          <div class="user-role min ">
                                             ...
                                             if($rowe['status'] == 'published'){
                                                echo '<span class="badge badge-success text-left">'.strtoupper($rowe['status']).'</span>';
                                             }elseif ($rowe['status'] == 'pending') {
                                                echo '<span class="badge badge-warning text-left">'.strtoupper($rowe['status']).'</span>';
                                             }else{
                                                 echo '<span class="badge badge-danger text-left">'. strtoupper($rowe['status']).'</span>';
                                             } ?>
                                          </div>
                                       </a>
                                       <hr>
         ...
         <!-- ONLINE QUIZ TODAY -->
         <?php if($online_quiz_today->num_rows() > 0){ ?>
            <div class="ui-block paddingtel" >
            <div class="pipeline white lined-success" >
               <div class="element-wrapper" >
                           ...
                           <div class="users-list-w">
                              ...
                              if($online_quiz_today->num_rows() > 0){
                                 ...
                                 foreach($quizzes as $rowe): ?>
                                    ...
                                    $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
                                    $quiz_id = $rowe['online_quiz_id'];

                                    $failed = $this->db->query("SELECT * from tbl_online_quiz_result where online_quiz_id = '$quiz_id' and result = 'fail'")->num_rows();

                                    $passed = $this->db->query("SELECT * from tbl_online_quiz_result where online_quiz_id = '$quiz_id' and result = 'pass'")->num_rows();
                                    ...
                                    <div class="user-name">
                                       <a href="<?php echo base_url().'teacher/quiz_results/'.$rowe['online_quiz_id']; ?>" target="_blank">
                                          <h6 class="user-title min"><?php echo $rowe['title'];?> <br>
                                             <span class="badge badge-success"><span class="fa fa-clock"></span> <?php echo date('g:i A', strtotime($rowe['start_date'])) . ' - ' . date('g:i A', strtotime($rowe['end_date'])); ?> </span>
                                             <span class="badge badge-primary" title="Total Examiners"> <span class="fa fa-users"></span> <?php echo $total_examiner;?></span>
                                          </h6>
         ...
         <!-- Activities TODAY -->
         <?php if($activity_today->num_rows() > 0){ ?>
            <div class="ui-block paddingtel" >
            <div class="pipeline white lined-success" >
               <div class="element-wrapper" >
                  <h6 class="element-header"><?php echo get_phrase('Activities Today');?> &nbsp; <span class="badge badge-primary float-right"><b><?php echo number_format($activity_today->num_rows(),0); ?></b></span> </h6>
                  <div class="full-ch at-w" id="results">
                     <div class="chat-content-w min">
                        <div class="chat-content min">
                           <div class="users-list-w">
                              <?php 
                              if($activity_today->num_rows() > 0){

                                 $quizzes = $activity_today->result_array();
                                 foreach($quizzes as $rowe): ?>
                                    <?php 
                                    ...
                                    $homework_code = $rowe['homework_code'];

                                    $total_deliveries = $this->db->query("SELECT * from deliveries where homework_code = '$homework_code'")->num_rows();

                                    ?>
                                       
                                    <div class="user-name">
                                       <a href="<?php echo base_url().'teacher/homework_details/'.$rowe['homework_code']; ?>" target="_blank">
                                          <h6 class="user-title min"><?php echo $rowe['title'];?> <br>
                                             <span class="badge badge-success" title="Time End">
                                                <span class="fa fa-clock"></span> <?php echo date('g:i A', strtotime($rowe['time_end'])); ?> 
                                             </span>
                                             <span class="badge badge-primary" title="Total Deliveries"> <span class="fa fa-users"></span> <?php echo $total_deliveries;?></span>
                                          </h6>
                                          <div class="user-role min ">
                                             <span class="badge badge-info text-left  txt_ellipsis" title="<?php echo $class.' - '.$section;?>"><?php echo $class.' - '.$section;?></span>
                                             <span class="badge badge-danger text-left txt_ellipsis" title="<?php echo $subject;?>"><?php echo $subject;?></span>
                              ...
                              <?php }else{ ?>
                                 ...
                                 <h4 class="text-center"><span class="fa fa-times"></span> No activity for today. </h4>
         ...
         <!-- LIVE CLASS TODAY -->
         <?php if($live_class_today->num_rows() > 0){ ?>
            <div class="ui-block paddingtel" >
            <div class="pipeline white lined-success" >
               <div class="element-wrapper" >
                  <h6 class="element-header"><?php echo get_phrase('Live Class Today');?> &nbsp; <span class="badge badge-primary float-right"><b><?php echo number_format($live_class_today->num_rows(),0); ?></b></span> </h6>
                  <div class="full-ch at-w" id="results">
                     <div class="chat-content-w min">
                        <div class="chat-content min">
                           <div class="users-list-w">
                              <?php 
                              if($live_class_today->num_rows() > 0){

                                 $quizzes = $live_class_today->result_array();
                                 foreach($quizzes as $rowe): ?>
                                    <?php 
                                    ...
                                    ?>
                                    <div class="user-name">
                                       <a href="<?php echo base_url().'teacher/live_class/'.base64_encode($class_id."-".$section_id."-".$subject_id);?>" target="_blank">
                                          <h6 class="user-title min"><?php echo $rowe['title'];?> <br>
                                             <span class="badge badge-success" title="Time End">
                                                <span class="fa fa-clock"></span> <?php echo date('g:i A', strtotime($rowe['start_time'])); ?> 
                                             </span>

                                             <?php 

                                             if($rowe['host_id'] == 1){
                                                echo '<span class="badge badge-info">USING ZOOM </span>';
                                             }else if($rowe['host_id'] == 2){
                                                echo '<span class="badge badge-primary">USING JITSI MEET </span>';
                                             }else{
         ...
         <div class="pipeline white lined-success">
         <div class="element-wrapper" >
         <h6 class="element-header"><?php echo get_phrase('online_users');?> <span class="badge badge-primary float-right"><b><?php echo $user_counter; ?></b></span></h6>      
         <?php 
            if(!isset($_SESSION)){
                session_start();
            }
         ...
         <?php  
            ...
            $usuarios = $this->crud_model->get_online_users('teacher');
            foreach($usuarios as $row):
            ?>
         <div class="user-w with-status min status-green">
         <div class="user-avatar-w min">
         <div class="user-avatar" >
         <img alt="" src="<?php echo $this->crud_model->get_image_url($row['type'], $row['user_id']);?>">
         </div>
         </div>
         <div class="user-name">
         <h6 class="user-title min"><?php echo $this->crud_model->get_name($row['type'],$row['user_id']);?></h6>
         <div class="user-role min">
         ...
         <?php 
            ...
            $var1 = 0;
            $login_id = $this->session->userdata('login_user_id');
            $grade_id = $this->db->query("SELECT class_id FROM class where teacher_id = '$login_id'")->row()->class_id;
            $birthdays = $this->crud_model->get_birthdays($grade_id,'teacher');
         ...
         <a href="<?php echo base_url();?>teacher/birthdays/" class="btn btn-warning btn-sm"><?php echo get_phrase('view_all_birthdays');?></a>
         </div>
         </div>
         </div>
         <?php endforeach;?>
