<?php
   $question_details = $this->db->get_where('tbl_question_bank_quiz', array('question_bank_id' => $param2))->row_array();
   ...
   $online_quiz_details = $this->db->get_where('tbl_online_quiz', array('online_quiz_id' => $question_details['online_quiz_id']))->row_array();
   
   $added_question_info = $this->db->get_where('tbl_question_bank_quiz', array('online_quiz_id' => $online_quiz_details['online_quiz_id']))->result_array();
...
<div class="modal-body">
   ...
   <div class="ui-block-content">
      <div class="row">
         <div class="col-md-12">
            <?php //echo form_open(base_url() . 'teacher/update_online_quiz_question/'.$param2.'/update'.'/'.$question_details['online_quiz_id'] , array('enctype' => 'multipart/form-data'));?> <?php //echo form_close();?>
            <form enctype="multipart/form-data" id="form" onsubmit="event.preventDefault();">
               ...
               <div class="form-group">
                 <div class="col-sm-12">
                    <button type="submit" class="btn btn-success btn-block" onclick="update_multiple_choices('<?php echo $param2;?>','<?php echo $question_details['online_quiz_id'];?>');"><?php echo get_phrase('Update');?></button>
                 </div>
              </div>
            </form>
         </div>
...
<script type="text/javascript">
   ...
   function update_multiple_choices($param, $quiz_id){
      //alert($param +' '+ $quiz_id);

      $.ajax({
         url:'<?php echo base_url();?>admin/update_online_quiz_question/'+$param+'/update/'+$quiz_id,
         method:'POST',
         data:$("form#form").serialize(),
         cache:false,
         success:function(data)
         {
         window.location.href = '<?php echo base_url();?>admin/quizroom/'+$quiz_id;
         }
      });
   }
