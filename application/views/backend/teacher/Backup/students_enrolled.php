                      ...
                      <div class="aec-full-message">
                        <div class="container-fluid" style="background-color: #f2f4f8;">
                          ...
                          <div class="col-sm-12">
                            ...
                            <div class="row">
                              <div class="col col-lg-10 col-md-12 col-sm-12 col-12">
                                ...
                                <div class="card-box widget-user ui-block list">
                                  ...
                                  <div class="more" style="float:right;">
                                    ...
                                    <ul class="more-dropdown">
                                      <li><a href="javascript:void(0);" onclick=" exclude_student('<?php echo $row2['student_id']; ?>','<?php echo $class_id; ?>','<?php echo $section_id; ?>','<?php echo $subject_id; ?>','<?php echo $param_data; ?>')"><?php echo get_phrase('exclude');?></a></li>
  ...
  <div class="modal-dialog window-popup create-friend-group create-friend-group-1" role="document">
    <div class="modal-content">
      ...
      <form enctype="multipart/form-data" id="student_exclude" onsubmit="event.preventDefault();">
        <a href="#" class="close icon-close" data-dismiss="modal" aria-label="Close" onclick="load_student_enrolled('<?php echo $param_data; ?>')"></a>
        ...
        <div class="modal-body">
          ...
          <div class="form-group label-floating is-select">
            <label class="control-label"><?php echo get_phrase('student_list');?></label>
            <div class="select">
              <select name="stud_list" required="">
                <option value=""><?php echo get_phrase('select');?></option>
                <?php 
                  $classes = $this->db->query("SELECT a.`student_id`, CONCAT(a.`last_name`,', ',a.`first_name`) AS fullname, b.`class_id`, b.`section_id` FROM student a LEFT JOIN enroll b ON a.`student_id` = b.`student_id` LEFT JOIN class c ON b.`class_id` = c.`class_id` WHERE c.`department_id` = '4' ORDER BY a.`last_name` ASC")->result_array();
                  foreach($classes as $row):
                ?>
          ...
          <button type="submit" class="btn btn-rounded btn-success btn-lg full-width" onclick="add_subj_student('<?php echo $param_data; ?>')"><?php echo get_phrase('Add');?></button>
        </div> 
      </form>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  function load_student_enrolled($param){
    window.location.href = '<?php echo base_url();?>teacher/students_enrolled/' + $param;
  }

  function exclude_student($stud_id, $cl_id, $sec_id, $subj_id, $param) {

    swal({
      title: "Are you sure?",
      text: "You want to exclude this student to your subject?",
      type: "warning",
    ...
    function(isConfirm){
      ...
      {  
        ...
        //alert(stud_id +'-'+ cl_id +'-'+ sec_id +'-'+ subj_id +'-'+ $param);
        // var id = [];
        // // var user_type = [];
        ...
        $.ajax({
          url:'<?php echo base_url();?>teacher/exclude_student/',
          method:'POST',
          data:{cl_id:cl_id,sec_id:sec_id,subj_id:subj_id,stud_id:stud_id},
          cache:false, 
          success:function(data)
          {
            //alert(data);
